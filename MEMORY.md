...
⚠️ **The CI flake itself remains unreproduced** — shipped as *fix + diagnosis*, not as *confirmed root cause*. Nothing here can recreate the condition (CI ran 233s vs 103s locally; no transient DB/network error appears anywhere in the failing log). What changed is that a recurrence now names its branch in the log instead of surfacing as a bare missing stamp. XC-4 is also self-verifying now: it asserts `endMatchAction`'s own `{success:true}` and that its backdate hit exactly one row, and it backdates from `Date.now()` — the same clock `isHeldMatchReady` reads — so cross-clock skew is no longer a variable.
