-- ============================================================
-- C7: migrate_player_identity — repoint player_rivalries and
-- player_partnerships old -> new on reconnect (non-fatal, mirrors
-- the club_members block added in 20260630000003).
-- ============================================================
-- Both tables are directional ledgers keyed by
-- (club_id, player_id, {rival_id|partner_id}) — a migrating player
-- can appear on EITHER side of a row, so each table needs two
-- symmetric repoint passes (player_id side, then the counterpart
-- side). Blind UPDATEs are not safe here for two reasons:
--
--   1. PK collision: if the new identity already has a row against
--      the same opponent in the same club (not reachable through
--      reconnectPlayer() today, since the new auth id is always a
--      freshly minted anonymous user, but this RPC is general
--      infra and should not corrupt data if called under different
--      conditions) — a blind UPDATE would hit
--      player_rivalries_pkey / player_partnerships_pkey. Fixed by
--      merging counters into the existing new-identity row (summing
--      wins/losses/games, taking the later last_faced_at/
--      last_played_at) before deleting the now-redundant old row.
--
--   2. CHECK violation: both tables enforce player_id <> rival_id
--      (resp. partner_id). If old and new ever faced/partnered each
--      other directly, repointing one side would produce a
--      self-referencing row post-migration. Guarded by deleting any
--      old<->new pair row up front — the two identities are about
--      to become the same person, so a self-rivalry/self-partnership
--      row is meaningless and must not be created.
--
-- Order matters: guard -> merge -> delete-duplicates -> update-
-- remainder, per side, per table. By the time the counterpart-side
-- pass runs, no row has player_id = p_old_user_id left (the first
-- pass already cleared them), so the two passes cannot interfere.
-- ============================================================
