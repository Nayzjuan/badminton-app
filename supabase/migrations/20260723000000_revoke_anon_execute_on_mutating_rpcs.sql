...
--
-- There is no ordering hazard and no code change is required for this file.
-- service_role access is unchanged, so it may be applied before or after any
-- deploy. Unlike 20260722010001 it does not have an "apply after the deploy"
-- half — apply it NOW.
--
-- ── SCOPE ─────────────────────────────────────────────────
...
revoke execute on function public.swap_teams_in_active_match(uuid, uuid, uuid, uuid, text, boolean, uuid) from public, anon, authenticated;
