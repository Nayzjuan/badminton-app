-- ============================================================================
-- C2 correctness fix: migrate_player_identity must repoint club_milestones.
--
-- club_milestones (created 2026-07-04, first_to_100) has
--   player_id uuid NOT NULL REFERENCES profiles(id) ON DELETE CASCADE
-- but migrate_player_identity (last touched 2026-07-01) was never retrofitted.
-- On a non-organizer reconnect the function ends with
--   DELETE FROM profiles WHERE id = p_old_user_id
-- which cascade-deletes the milestone row, permanently vacating the
-- UNIQUE(club_id, milestone) slot for the next crosser to wrongly claim.
--
-- Fix: add one guarded repoint block (same non-fatal BEGIN/EXCEPTION pattern as
-- the eight existing repoint blocks) before the final profiles DELETE.
-- UNIQUE(club_id, milestone) excludes player_id, so a blind UPDATE can never
-- conflict (only one row per (club, milestone) exists, held by one player).
--
-- CREATE OR REPLACE (no signature change) preserves GRANTs. Body reproduced
-- verbatim from the live prod definition + the single new block.
-- ============================================================================
...
BEGIN
  ...
  -- NEW (multi-tenant): re-point club authorship and invited_by old -> new.
  BEGIN
  ...
  -- C2 (2026-07-18): re-point one-time club milestones (first_to_100) old -> new.
  -- Without this the final profiles DELETE cascade-deletes the milestone row and
  -- permanently vacates the UNIQUE(club_id, milestone) slot. UNIQUE excludes
  -- player_id, so a blind repoint can never conflict.
  BEGIN
