-- ============================================================
-- migrate_player_identity — repoint clubs.created_by and
-- club_members.invited_by old -> new on reconnect (non-fatal,
-- same pattern as the club_invites block added in
-- 20260701000016).
-- ============================================================
-- Found via a full audit of every FK column referencing profiles(id)
-- after the club_invites gap surfaced live: two more multi-tenant
-- columns were never retrofitted into this function —
--   clubs.created_by         (the club creator's profile)
--   club_members.invited_by  (whoever generated the invite a member
--                              joined through)
-- Neither column carries a uniqueness constraint, so — like
-- club_invites — a blind two-row UPDATE is safe: no merge/dedupe
-- needed. clubs.created_by was actively blocking reconnect for any
-- club's original creator (confirmed live: reconnecting as
-- "QA Owner Test", the creator of qa-member-test-club, failed with
-- clubs_created_by_fkey before this fix).
-- ============================================================
