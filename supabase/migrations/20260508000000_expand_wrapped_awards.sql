-- ============================================================
-- Migration: Expand Session Wrapped award catalogue (27 → 51)
-- ============================================================
-- Adds 24 new awards across 6 categories. Refines the existing
-- `sniper` award to a 5–7-pt margin band so it is mutually
-- exclusive with the new `heartless` (≥8 pt) award. Tier-replaces
-- `sunset_surge` with `clean_sweep` when the player won all of
-- their last 3 games.
--
-- Other changes:
--   • Refreshes `v_alltime_leaderboard_mat` at the top of the
--     RPC so `century_club` and `the_veteran` see fresh data.
--   • Extends the `_wrapped_stats` temp table with ~25 new
--     computed columns covering margin, partner diversity,
--     opponent pair history, skill-vs-opponent, mixed-level
--     wins, first/last match flags, and all-time milestones.
--   • Pre-computes cross-player relationships (partner counts,
--     opponent pair history, redemption arcs, skill matchups,
--     partner/opponent overlap) as supporting CTEs so the
--     per-player award loop stays simple IF blocks.
--
-- Backwards-compatible: signature, return type, and grants are
-- unchanged — full replacement of the existing function body.
-- ============================================================
...
BEGIN
  ...
  FOR v_player IN SELECT * FROM _wrapped_stats LOOP
    ...
    -- NEW: The Closer — won the very last game (min 3 games so 1-game players don't get it)
    ...
    -- Friendly Fire — played against ≥2 people you also partnered with
    -- Threshold ≥2 prevents awarding to every player in a small session
    -- where nearly everyone ends up on both sides at some point.
    ...
    -- NEW: The Warm-Up Act — lost all games by ≥6 pts avg, ≥3 games, no wins
    -- Tier-replaces participation_trophy: warmup_act is more specific and punchy;
    -- a player who qualifies doesn't need the generic "you showed up" consolation.
    IF v_player.wins = 0
