...
async function createBot(displayName: string, skill = "intermediate"): Promise<BotPlayer> {
  const db = adminDb();
...
async function setOrganizerVip(organizerUserId: string, tag: string | null, theme: string | null) {
  await adminDb()
    .from("profiles")
...
test.describe("MatchAlert — [B] On-deck position 2 shows #2 copy", () => {
  test("eyebrow reads #2 On Deck and subline mentions 1 match ahead", async ({ browser }) => {
