...
test.describe("Tap-to-Swap v2 — [G] VIP tag preserved", () => {
  test("VIP tag on player pill remains visible in picking mode and after swap", async ({
    ...
    try {
      ...
      const alicePill = page.getByTestId(`player-pill-${seeded.players.alice.userId}`);
      await expect(alicePill.getByText("VIP").first()).toBeVisible({ timeout: 8_000 });
      ...
      await expect(alicePill.getByText("VIP").first()).toBeVisible();
      ...
      await expect(alicePillM2.getByText("VIP").first()).toBeVisible({ timeout: 5_000 });
    } finally {
      await context.close();
    }
  });
