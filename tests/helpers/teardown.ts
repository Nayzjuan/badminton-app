...
export interface TeardownResult {
  ...
  queueStatusEventsDeleted: number;
...
export async function resetSandboxSession(): Promise<TeardownResult> {
  ...
  // ── Step 6b: Delete queue_status_events ───────────────────
  // Exactly the shape of match_events above, and it arrived without anyone
  // noticing: 20260815 added an AFTER UPDATE OF status trigger on
  // queue_entries that writes one audit row per transition, and
  // queue_status_events has NO foreign key to queue_entries — session_id and
  // player_id are plain uuid columns — so the queue delete cannot cascade it.
  // Every E2E run drafts, seats and re-queues bots dozens of times, and
  // repairSandboxState resets statuses on top of that, so the rows are
  // generated by the harness itself and then left behind permanently. This
  // is the match_events leak a second time; it is caught here before it can
  // become another count in a production table.
  //
  // Runs AFTER the queue_entries delete on purpose: a DELETE does not fire an
  // UPDATE OF status trigger, so the step above writes nothing this could
  // miss. It is not the last step — courts, bot users and the session reset
  // still follow — but the queue rows are already gone by here, so no later
  // step can produce a status transition either. This path never calls
  // repairSandboxState (only softResetSandboxSession does, which cleans up
  // after itself at its own step 5b); the rows an earlier spec's soft reset
  // left behind are session-scoped, so this delete sweeps them regardless.
  // Scoped by session_id, which the sandbox-name guard has already validated.
  const { count: queueStatusCount, error: queueStatusErr } = await db
    .from("queue_status_events")
    .delete({ count: "exact" })
    .eq("session_id", sessionId);

  if (queueStatusErr) {
    throw new Error(`[teardown] Failed to delete queue_status_events: ${queueStatusErr.message}`);
  }
  ...
  const result: TeardownResult = {
    ...
    queueStatusEventsDeleted: queueStatusCount ?? 0,
  ...
  console.log(
    `[teardown] ✓ Reset session ${sessionId}: ` +
      ...
      `${result.queueStatusEventsDeleted} queue status events`
  );
...
export async function softResetSandboxSession(): Promise<void> {
  ...
  // Step 5b: Delete queue_status_events — see the long note in
  // resetSandboxSession. No FK to queue_entries, so the delete above cannot
  // cascade it, and this path calls repairSandboxState first, which drives
  // playing/drafted/on_deck back to waiting and writes a row for each.
