...
It reaches two awards, one per CTE: `consistent_dominator` gates on `cs_prior_dominant_sessions` from `prior_sessions_ranked`, and `momentum` on `cs_prior_win_streak` from `prior_carry`. A 3-0 infrastructure session is a 100% night and a 3-game streak — everything both gates want.
...
**Pinned by Suite XS-7**, which fails against the pre-migration function (`consistent_dominator` and `momentum` both appear from a hidden night), and **XS-8**, its counterweight: the same two nights with the first left visible must still award both, so a filter that emptied the CTEs instead of bounding them cannot pass.
