...
- **Court management**: Add, rename, toggle status (`available` / `closed`), remove (confirmation dialog). Every one of these surfaces its failure via inline card error and/or toast banner; none fails silently. Add carries two distinct failure paths, because a server action can fail in two ways: it can *return* `success: false`, and it can *reject*. The returned case in production is a duplicate name — `courts` holds `UNIQUE (session_id, name)`, so re-adding an existing name comes back as Postgres `23505`, which `addCourtAction` maps to prose naming the court instead of the index. The rejecting case is any 500, which the `{ success }` branch never sees; `handleAddCourt` catches it, explains it, and clears the spinner in a `finally` so the button cannot strand on “Adding…”. Handlers: `handleAddCourt`, `handleUpdateCourtStatus`, `handleRemoveCourt` in `active-courts.tsx`; the duplicate mapping is in `addCourtAction` (`src/app/actions/courts.ts`).
