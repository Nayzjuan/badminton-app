...
| `checkout_player_cleanup_drafts(p_session_id, p_player_id)`     | Self-checkout: removes player from unpublished drafts, cancels drafts with <4 remaining, returns cancelled match IDs. `service_role` only.                                                                                                                                                                                           |
| `join_queue(p_session_id, p_player_id)`                         | Atomic queue join with Inherited Games floor; handles first-time + returning player. `service_role` only.                                                                                                                                                                                                                            |
| `publish_match(p_match_id, p_session_id, p_user_id)`            | Atomic publish: auth check, conflict guard, HAS_LEFT_PLAYERS guard, promotes 'drafted'→'on_deck'.                                                                                                                                                                                                                                    |
| `publish_all_drafts(p_session_id, p_user_id)`                   | Batch publish all unpublished pending matches.                                                                                                                                                                                                                                                                                       |
| `clear_on_deck_match_atomic(p_match_id, p_session_id)`          | Atomically restores players to 'waiting' and deletes a pending match. **Lock order: queue_entries → matches.** Returns player ID array for broadcast. `service_role` only. Migration 20260512200004.                                                                                                                                 |
| `remove_player_from_queue_organizer(p_session_id, p_player_id)` | Organizer kick: removes player from all pending match rosters, cancels under-strength matches, restores other players, sets status='left'. **Lock order: queue_entries → matches.** Returns affected match IDs. `service_role` only. Migration 20260512200002.                                                                       |
| `revert_match_to_active(p_match_id, p_session_id)`              | Atomically reverts a completed match to 'in_progress' and bulk-reverts all 'waiting' roster players to 'playing' via JOIN UPDATE. `service_role` only. Migration 20260512200003.                                                                                                                                                     |
...
**Suites (156 passed across 16 files — last updated 2026-05-12):**

| Suite                                     | Tests | Covers                                                                                                                                                                                                                                                                                                 |
| ----------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `health.test.ts`                          | 4     | Harness smoke — connectivity, factory, cleanup                                                                                                                                                                                                                                                         |
| `auth.real.test.ts`                       | 1     | Mock drift detector — real auth roundtrip                                                                                                                                                                                                                                                              |
| `matchmaking.test.ts`                     | 10    | Engine: draft mode, partnership cap, mixed-skill, Red Zone, draft cap, TOCTOU, `hasDraftsBlocking`, **paused-player exclusion (Test 9)**, **partnership-cap saturation no-match (Test 10)**                                                                                                            |
...
| `score-submission.test.ts`                | 13    | `endMatchAction` cascade + **`cancelMatchAction` (F-cancel-1..5): status flip, games_played NOT incremented, CAS double-cancel guard, 'left' players preserved**                                                                                                                                       |
...
| `drafted-status.test.ts`                  | 14    | Drafted-status lifecycle, publish-with-left guard, mixed status close-session                                                                                                                                                                                                                          |
| `rpc-behaviors.test.ts`                   | 15    | Suite H (`elevate_to_organizer`) + Suite I (`rejoin_queue`) RPC behavior                                                                                                                                                                                                                               |
| `session-lifecycle.test.ts` (Suite K)     | 13    | `createSession` (passcode unique, length, trigger, unauth), `joinAsCoOrganizer` (idempotent, primary-org rejection, generic invalid msg, closed-session rejection), `toggleAutoMatchmaking` (atomic flip, organiser auth)                                                                              |
| `queue-join.test.ts` (Suite L)            | 8     | `joinQueueAction` happy + Inherited Games floor, returning 'left' rejoin, drafted/on_deck/playing rejection, unauth, `checkoutPlayer` draft cleanup                                                                                                                                                    |
| `manual-and-swap.test.ts` (Suite M)       | 13    | `createManualMatchAction` happy + 3 negative paths, `swap_player_in_match` sticky-rule (manual stays, modified stays), `swapPlayerInMatch` action guards, `swap_match_players` origin flips. M-5 unskipped after regression fix.                                                                       |
| **`match-update.test.ts` (Suite N, NEW)** | 12    | `updateMatchDetails` (score edit, revert-to-active, non-org rejected, invalid ID, revert-decrements-only-waiting), `clearOnDeckMatch` (happy, 'left' player preserved, non-org, in_progress rejected), `reorderOnDeckMatches` (happy, non-org, invalid ID). Validates all three F-fix RPCs end-to-end. |
...
26. **`swap_player_in_match` Step f regression (detected and FIXED 2026-05-12)** — Migration `20260502000000_match_origin_tracking.sql` originally added Step f to `swap_player_in_match` (promote `matches.origin` from `'auto'` → `'modified'` after a single-player swap, sticky rule). Two later rewrites silently dropped Step f: `20260509000000_swap_player_draft_aware.sql` and `20260511000001_swap_player_drafted_status.sql`. Effect (before fix): an organiser-initiated single-player swap on an engine-generated draft never flipped its origin to `'modified'`, breaking the audit trail and the sticky-manual semantics that `v_match_history` relies on. **FIX:** Migration `20260512000000_restore_swap_player_origin_flip.sql` re-issues the function with the same `(UUID, UUID, UUID, UUID, TEXT, BOOLEAN)` signature, Steps a–e copied verbatim from `20260511000001`, plus the restored Step f (`UPDATE matches SET origin='modified' WHERE id=p_match_id AND origin='auto'`). Verified by integration test `M-5` in `tests/integration/manual-and-swap.test.ts` (passing). **`swap_match_players` (the match↔match RPC) was never affected** — its origin flip from migration `20260502000000` is intact (verified by Suite M tests M-11 and M-12).
27. **Dual-state player race conditions — FIXED (2026-05-12, migrations 200000–200004)** — Five paths existed where a player could appear simultaneously in `queue_entries` with one status and in `match_players` with an inconsistent status (e.g. `on_deck` in queue but assigned to two published pending matches simultaneously). Root causes and fixes:
    - **F1 (P0 — FIXED):** `swap_player_in_match` RPC had no DB-level guard on the incoming player — two concurrent swaps could both pass the JS-layer status check and insert the player into two matches. Fixed in `20260512200000` by adding `SELECT FOR UPDATE` on the incoming player's `queue_entries` row (Guard A), a post-lock conflict check (Guard B), and `FOR UPDATE` on the match row (Guard C). **Global lock order: `queue_entries` → `matches` across all RPCs.**
    - **F2 (P1 — FIXED):** `clearOnDeckMatch` non-atomic: restoring players to 'waiting' and deleting the match were separate statements. Fixed by `clear_on_deck_match_atomic` RPC (`20260512200004`) which performs both atomically, with `queue_entries` locks acquired before the match lock to prevent deadlock with F4's RPC.
    - **F3 (P1 — closes with F1):** Downstream of F1 — required concurrent swaps as prerequisite.
    - **F4 (P2 — FIXED):** Organizer kick (`removeFromQueue`) did a raw client UPDATE with no `match_players` cleanup. Fixed by `remove_player_from_queue_organizer` RPC (`20260512200002`) which atomically removes the player from all pending match rosters (cancelling under-strength matches) before setting status='left'. New `removePlayerFromQueue` server action wraps it; hook callback updated.
    - **F5 (P2 — FIXED):** `updateMatchDetails` revertToActive path had N+1 round trips and 3 silent failure modes (matchPlayers fetch fail → false success; individual fetch fail → skip; Promise.all throw → partial state). Fixed by `revert_match_to_active` RPC (`20260512200003`) which atomically reverts match + bulk-updates all 'waiting' roster players via single JOIN UPDATE.
