...
- `session_closed` — redirects all connected players to `/wrapped/{sessionId}/{playerId}`. Optional `actorId` / `actorName` (omitted when unknown). Organizer boards with `creditCloser` toast "{name} closed the session."; players keep the awards copy.
- `drafts_published` — `{ actorId, actorName, count }` → organizer-only toast that a co-organizer published drafts. Not handled on the player hook. Fired only on a real SUCCESS transition (not `ALREADY_PUBLISHED`, not engine auto-publish). Publish All is one event with count.
- `auto_matchmaking_toggled` — `{ isOn: boolean }` → syncs auto-matchmaking state to all co-organizers (bypasses the sessions RLS SELECT policy that would silently drop postgres_changes for non-creator organizers). The acting tab toasts from `handleToggleAuto`; other tabs toast when `liveAutoMatchmaking` flips and local `pendingAuto` does not match.
...
| `created_at`               | `timestamptz`         |                                                                                |
| `ended_at`                 | `timestamptz \| null` | Close-claim stamp: set while `is_active` is still true, then `is_active` flips after Wrapped. Live vs closed is `is_active`, not this column. |

**Trigger:** `on_session_created` → `handle_new_session()` — auto-inserts a `session_organizers` row for `created_by`.

**Parked — not unique per club while active.** `createSession` refuses a second active session in the same club only inside a 10-minute SELECT-then-INSERT window. A sub-commit-latency double-create can still open two live nights. A partial UNIQUE `(club_id) WHERE is_active AND NOT is_hidden` would close that; it is not shipped because overlapping nights may be allowed on purpose. See MEMORY.md.
...
**Consecutive-partnership ban** (last match only, hard): `deriveLastPartners(snapshot)` — sibling of `deriveLastOpponents`, same newest-first / pending-included / malformed-roster contract, produced from the same snapshot walk (`deriveLastSides`). `isLastPartnership` is symmetric: if **either** player still has the other as last teammate, the split is illegal (a sit-out does not clear the other player's last partner). They may still face each other immediately, and they may partner again later in the session while the session count is still 1. The seater applies this **before** the freshness ladder (including `usedCapOverride`), so last game's teammates are never reseated. Candidates are **not** pre-filtered by last partner — that would also stop them facing each other. If every balanced split of a four is a last-partner repeat, `snakeDraft` returns `null`, the window expands, and a stall broadcasts `capSaturationReason: "consecutive"` (distinct from `"session_cap"`). Manual organizer assignment is the only bypass.

#### Team Draft (`snakeDraft` / `rotatedDraft`)

Sort all 4 players DESC by skill, then apply a **two-pass approach** for partner freshness over splits that are **not** last-partner repeats:

- **Pass 1 (prefer fresh)**: Try splits in skill-balance order where **both team pairs have `count = 0`** (never been partners).
- **Pass 2 (fallback)**: Try splits in skill-balance order where both pairs are `< MAX_PARTNERSHIP_REPEATS` **and** neither pair is last game's teammates. A count-1 pair from two games ago is still legal.

`selectSplit` drops last-partner splits from the pool before any pass, including `ignorePartnershipCap`. `usedCapOverride` may still seat an over-cap split that is *not* a last-partner repeat; it returns `null` rather than reseating last game's teammates.
...
- `deriveLastOpponents(snapshot)` / `deriveLastPartners(snapshot)` — who each player faced across the net, and who they partnered with, **in their LAST match only**. Both maps come from one walk (`deriveLastSides`). Opponent map is whole-pool, not anchor-relative: **79.3% of back-to-back opponent repeats are between two NON-anchor co-players**, which `deriveOverlapMap` structurally cannot see at any weight. Partner map is the consecutive-partnership ban input. Newest-first (first write wins, relying on the snapshot's `created_at DESC, id DESC` contract) with **no lookback window** — a player's last match counts however long ago it was. A malformed roster (not exactly two teams of two) marks its players **seen with an empty set** rather than being skipped, so an older match can never leak in as someone's "last". Bounded by `SESSION_MATCH_SNAPSHOT_CEILING`, so O(≤200 × 4) with a per-player early-out.
...
- **Readiness** (`recomputeHeldReadiness`, before promote on end/cancel): roster-integrity downgrade (N-2), source-null cancel via `clear_on_deck_match_atomic` (R3-B), **hold-age cancel** (`heldDraftExpired`, above), stamp `held_ready_at` once source completed AND (≥1 promotion since freed OR `CROSS_COURT_REST_FALLBACK_MINUTES` timer — C-5, no counter column).
  - **A failed read is never an answer.** Two of the branches above are destructive — N-2 clears the pull, R3-B cancels the draft and releases three parked players — and both are decided by a `select` whose error was once discarded, so a transient failure did the same thing as a departed body. Each read now separates *absent* from *unreadable*, which `maybeSingle` already distinguishes (a genuinely missing row is `data` null **with** `error` null): on an error the draft is left exactly as it is and skipped for one pass, since every caller re-enters at the next lifecycle event. Two reads are deliberately asymmetric. The promotion count still falls back to `0`, because miscounting only *delays* a stamp that the rest-fallback arm resolves anyway. The `auto_publish` read instead abandons the whole pass: defaulting it to draft mode is the one unrecoverable wrong answer, since a session really in auto mode would get a draft stamped ready and never published, and auto mode hides both the drafts section and publish-all (`on-deck-panel.tsx`), leaving an orphan no organizer can finish or clear — the state the AP2/AP3 branch exists to prevent, reached through a failed read instead of a refused publish. Every branch that leaves `held_ready_at` null now says so: `CC-RDY-ERR1`…`CC-RDY-ERR5`, and `rg -n 'console.warn' src/app/actions/matchmaking.ts` locates them. That logging immediately earned itself: a CI run of XC-4 failed on a **502 from the API gateway on the stamp UPDATE** — after the readiness math had resolved — which had been surfacing as a bare missing `held_ready_at` and was not attributable to any branch while every branch was silent. **Readiness is best-effort by design and no retry was added here:** a failed stamp is logged and left for the next lifecycle event, which the engine's held-draft heartbeat makes dense in a live session. XC-4 therefore drives the pass up to 3× rather than asserting one round-trip, which is the contract the code actually offers; the stamp is idempotent (`.is("held_ready_at", null)`) so a retry cannot double-write.
...
- **Publish All**: batch-publishes every draft in the on-deck panel. A successful publish (single or all) emits `drafts_published` so other organizer tabs toast; `ALREADY_PUBLISHED` and engine auto-publish do not.
...
- `MATCH_NOT_ACTIVE` → match is no longer in_progress, **or does not belong to this session** — close sheet + info toast
- `PLAYER_NOT_IN_MATCH` → player already moved — close sheet + named info toast
- `PLAYER_UNAVAILABLE` → queue player taken — keep sheet open, re-pick
- `ONDECK_MATCH_STARTED` → on-deck match promoted mid-confirm — close sheet + warning toast
- `FILL_PLAYER_UNAVAILABLE` → fill player taken — keep fill picker open
- Realtime close reads the live `activeMatches` list, not the `open()` snapshot.
...
**Two timeouts, and the order between them is the point.** The component and hook suites spend
almost all of a test inside `userEvent` and `waitFor`, and under CPU contention several of them
failed with a timeout or a query error rather than a failed assertion — i.e. they went red for a
reason unrelated to the behaviour they name. `tests/setup/jest-dom.ts` raises RTL's
`asyncUtilTimeout` (default 1000 ms) and `vitest.config.ts` raises `testTimeout` above it. The
Vitest ceiling MUST stay above the RTL budget: if it does not, a `waitFor` that is about to report
a real failure is cut off first and the run reports the wrong reason. Raising them is not the fix
for a flaky test, only the headroom that lets the real fix be seen — the actual causes were a
synchronous `getBy*` for a label that only appears after a pending state resolves, typing into an
input still `disabled={isPending}`, waiting on a spy that fires before React commits, and an exact
equality on a value derived from the wall clock.
...
| `oauth-actions.test.ts` | **Suite OA (unit) — `src/app/actions/oauth.ts`.** Note the prefix is reused by `organizer-alerts.test.ts` in the integration table below; they are different lanes and different suites. Pins the dark-launch flag as a strict case-sensitive compare (six near-miss values must all leave the feature dark and construct no client), the session gate's ORDER inside `linkWithGoogle`, and that the actions actually route `next` through `safeNext` — asserted on the argument `signInWithOAuth` received, because "safeNext is correct" (Suite SN) and "the action calls safeNext" are two different claims and only the second one ships |
| `rename-actions.test.ts` | **Suite RN — the rename availability ladder and the commit RPC.** `rename-screen.tsx` branches on `code`, not on `message`, so every rung has to keep its own code; a test asserting only `success: false` cannot see two rungs collapse into one. The negatives assert what did NOT run — no service client for an unauthenticated or malformed caller, no global-uniqueness read once the persisted-collision rung has already refused — because the ladder's ORDER (auth → Zod → collided_name → uniqueness) is invisible to every happy-path assertion |
| `upcoming-match-actions.test.ts` | **Suite UM — `getUpcomingHeldDraft`,** the one deliberate hole in the held-draft firewall: it reads with the service client and reports a fact about the caller. Four filters are all that separate "am I reserved?" from "who else is reserved, in any session, in any state?" — `session_id`, `status`, `is_held`, `contains(pulled_player_ids, [user.id])` — so UM asserts the column=value pairs rather than their presence, and UM-10 proves the bound value TRACKS the authenticated identity instead of merely matching the fixture |
| `dev-tools-guard.test.ts` | **Suite DV — `src/app/actions/dev.ts`,** the most destructive module in the repo: `clearSessionData` deletes a session's matches, `match_games`, `match_players` and `queue_entries` through the service client, with no RLS and no organizer check behind it. Every negative flips exactly ONE of `requireAuth`'s three layers — `NODE_ENV`, `DEV_TOOLS_ENABLED` as an exact string, the auth check — leaving the other two open, so "this layer works" is distinguishable from "something, somewhere, said no", and each asserts the service client was never constructed. Deliberately absent from `vitest.config.ts` `coverage.include`; the reason is recorded there |
| `wrapped-actions.test.ts` | **Suite WR — `dismissWrappedIntro`,** a one-way door: there is no un-dismiss, so a wrong binding permanently removes a player's recap. WR pins the update to `session_id` AND `player_id` AND `intro_dismissed_at IS NULL`, and pins that `dismissed` reports whether a row was really stamped — the field that makes the no-row case visible, and the one a `success`-only test would let regress to a constant `true`. WR-1 asserts the USER client is the one used; whether the RLS policy behind it is correct is a database question and is out of scope here by design |
| `client-ip.test.ts` | **Suite CI — `getClientIp`,** which produces the `p_ip` argument of two fail-closed rate-limit RPCs. `reconnectPlayer` runs *before* the caller has any identity — `signInAnonymously` is a live path, so an attacker mints a fresh subject per guess and the subject arm never fires. The IP arm is the whole guard there. CI pins WHICH hop is taken from a multi-hop `x-forwarded-for` (leftmost; the rightmost collapses a whole venue into one bucket), that `x-real-ip` is not even read while `x-forwarded-for` is usable, and that the catch degrades to `null` — both callers treat a gate error as a denial, so an escaping exception takes reconnect offline for everybody |
| `rpc-utils.test.ts` | **Suite RU — `isRpcNotFound`,** nine lines that choose between an atomic RPC and its pre-atomic JS fallback. Migrations here are applied by hand, so every action wrapping a new RPC ships before the function exists. Too permissive and a real domain error silently runs the fallback the RPC was written to replace, reintroducing its TOCTOU race with the caller still seeing success; too strict and a not-yet-applied migration becomes a hard user-facing failure. RU asserts EXACT equality on `code` — not substring, not case-insensitive, not the message text, and no throw on `null`, which is the ordinary shape of success. Call sites: `rg -n 'isRpcNotFound\(' src/` |
| `leaderboard-refresh.test.ts` | **Suite LR — `shouldRefreshLeaderboard`,** the module-level debounce in front of a materialized-view rebuild that spans every club, fired without `await` by both its callers (`rg -n 'shouldRefreshLeaderboard\(' src/`), so nothing downstream ever observes how often it ran. LR-6 is the off-by-one the file exists for: the source stamps AFTER the guard returns, and moving that assignment one line up means a club completing a match every 20 s never refreshes again. The stamp is module state, so every test re-imports the module under fake timers; LR-1 and LR-9 exist to prove that isolation works, since an order-dependent suite would be a defect the tests introduced rather than found |
| `trailing-debounce.test.ts` | **Suite TD — `trailingDebounce`,** fifteen lines underneath every realtime refetch in the app (`rg -n 'trailingDebounce\(' src/`; `use-session-data` alone builds three in one effect). TD pins that there is NO leading edge, that the window is measured from the LAST call — a first-call anchor fires mid-burst, landing between the `matches` INSERT and its `match_players` rows and rendering a live match with nobody on the court — that each instance owns its own timer rather than sharing one, and that `cancel()` is not a no-op, because the subscription cleanups call it |
| `vip-config.test.ts` | **Suite VC — `getVipThemeConfig` / `isVipTheme`.** `profiles.vip_theme` is free-form text with no CHECK constraint and no enum, and `vip-tag.tsx` branches on truthiness alone, so a truthy-but-field-less return produces a syntactically perfect class attribute with nothing in the styling slot — unreadable over the court graphic and flagged by nothing. **VC-9/VC-10 pin a KNOWN DEFECT rather than a fix:** `isVipTheme` tests `theme in VIP_THEMES` and `in` walks the prototype chain, so `"toString"`, `"constructor"` and `"__proto__"` all pass as valid themes. The fix is one line (`Object.prototype.hasOwnProperty.call(...)`); those two tests are the ones to flip when it lands |
| `oauth-provision.test.ts` | **Suite OP — the service-role `profiles` write that runs on every Google sign-in,** with RLS bypassed, no policy underneath it and no return value revealing what it did. OP counts WRITES, not return values: only an unresolved stub (`needs_rename === true && collided_name === null`) may be touched, or a returning player's display name is overwritten with whatever Google currently says, on every login, forever. `profile.pin ?? generatePin()` is asserted with `generatePin` mocked to a known value distinct from the fixture, so "kept" and "re-minted" cannot be confused. The 23505 TOCTOU fallback is driven explicitly — it is the branch deciding whether the user reaches the rename gate or a dead callback |
| `session-notice-write.test.ts` | **Suite NW — the only writer of `session_notifications`,** deliberately best-effort: every recognised failure is swallowed, so a helper that never inserts, one that names the wrong columns, and one whose `isMissingNoticeTable` eats a permission-denied all look identical from outside — the action returns success and the organizer's inbox is simply empty forever. NW tests both halves of every degradation as a PAIR: the exact payload on the good path, AND that an *unrecognised* error does not get the same free pass. Assert only the first and a swallow-everything helper passes; only the second and a helper that never writes passes. Distinct from `tests/unit/session-notifications.test.ts` (selectors and copy) and integration Suite SN (the real-DB contract) |
| `wrapped-awards.test.ts` | **Suite WA — the only translation between an award slug minted by a migration and the words a player reads.** `AWARD_META` is hand-maintained TypeScript, so drift is certain: an unknown slug is ranked `common` by `?? "common"` and keeps its place in the list, while `WrappedAwardCard` returns `null` for it — so under the hard cap of six it silently EATS A CARD SLOT from a real award and the player sees five cards with no error anywhere (WA-8/WA-9). WA-4 asserts `RARITY_ORDER` and its stability tie-break in both directions so a passing run cannot be coincidence, and WA-20/WA-21 make the `val \|\| ""` refactor red, because `0` is a legal award value ("Won 0 of your last 3 games") |
| `validate-uuid.test.ts` | **Suite VU — `isValidUUID`,** the first-line guard in every server action (`rg -n 'isValidUUID\(' src/`). The failure it catches is a WIDENED predicate, not a broken one: a guard that rejects everything fails loudly on the first happy path anywhere, while one that accepts too much fails nowhere. VU-8 is the anchor test — drop the trailing `$` and the regex still matches the first 36 characters while a payload rides along. VU-17 pins the `typeof s === "string"` half, without which `RegExp.test` coerces an object whose `toString()` returns a valid id. VU-19 is a compile-time claim in a runtime test's clothes: if the signature stops being `s is string`, `npx tsc --noEmit` fails on the `.slice()` inside the narrowed branch |
| `lib-utils.test.ts` | **Suite LU — `cn` and `createUnknownProfile`.** LU-1 pins that tailwind-merge is in the chain at all: plain `clsx` returns `"p-2 p-4"` and leaves the outcome to stylesheet order. The stub is asserted with `toEqual`, never `toMatchObject`, because a subset match is satisfied by a stub that silently gained a field — and every value is a claim about what the UI shows: `beginner` is the LOWEST tier so a stub never seeds a player into a higher bracket, and `vip_tag` / `flagged_at` / `pin` are null so a stub never renders a VIP badge, never looks like a dupe, and never carries a credential. LU-9 pins that each call returns a FRESH object: three hooks enrich concurrently, and a cached singleton would make one mutated row mutate every unknown player on screen |
| `use-club-slug.test.ts` | **Suite CS — `useClubSlug`,** four lines of anchored regex that decide which club the app thinks you are in. Nothing else does: every club-scoped navigation site reads it back out of the pathname (`rg -n 'useClubSlug\(' src/`) and uses it as the ternary condition on the URL it builds. It fails in opposite directions — `null` on a club route silently ejects the user from the `/c/[clubSlug]` namespace mid-session, and a slug on a NON-club route makes the legacy pages build `/c/<x>/…` URLs out of a fragment that was never a club slug |
| `use-score-input.test.ts` | **Suite SI — the SETTLED mapping.** Every game that ends is a race: four players and the organizer can all reach for the score at once and the server's compare-and-swap lets one write land. The losers get `already_scored` or `match_cancelled`, and neither is their fault nor leaves them anything to do. Mapped to `{ error }` like any other failure, the card stays armed over a match that no longer exists, and the documented consequence is people recording the game a second way. SI pins that exactly those two codes map to `{ settled: true }` — which `useScoreForm` treats as terminal — and that every other failure keeps the form armed |
| `use-pair-counts.test.ts` | **Suite PC — the one hook whose entire error policy is "say nothing".** A `{ success: false }` and an outright rejection are both swallowed by design, because the repeat-pairing warning is advisory and must never block manual match creation. The cost is that a hook which loads NOTHING, EVER, is indistinguishable from a working one: `counts === null` is the documented "no data yet" state and every screen stays green. So every negative here carries a POSITIVE CONTROL inside the same test, and PC pins WHICH tuple array rehydrates into WHICH Map — swapping them changes no call count and no return shape |
| `use-visibility-refresh.test.ts` | **Suite VR — the phone-unlock recovery path,** the app's only recovery from the 07/25 failure class (§3.26): a mobile browser suspends JS and kills the WebSocket on screen lock, nothing tells the page, no WAL event will ever arrive again, and the last render stays on screen looking authoritative. VR pins GUARD ORDER — `visibilityState !== "visible"` is checked BEFORE the throttle stamp is written. Swap those two and the HIDING event, which always fires immediately before the one that matters, consumes the throttle budget, so the visible event is refused and recovery is delayed by a full window: exactly the incident the hook exists to prevent |
| `use-edit-match.test.ts` | **Suite EMH — the hook.** The prefix `EM` belongs to `edit-match-dialog-repeat.test.tsx`, which drives the same feature through the rendered DOM and therefore cannot advance a timer to a chosen millisecond, cannot observe a timer still armed at unmount, and cannot distinguish "no close scheduled" from "scheduled but not yet fired". Those are exactly the properties here: `closeTimerRef` is the only *detached* timer in the organizer surface, holding a `setTimeout` that closes the dialog `DIALOG_CLOSE_DELAY_MS` after a successful revert or notification-backed correction |
| `use-live-match-swap.test.ts` | **Suite LS — the CLIENT state machine of a live swap.** The server half is covered twice (Suite LMS drives the RPCs against a real database, Suite LSB proves the actions refuse a match id from another session), but neither renders the sheet, so neither can see which action runs, with which ids, or what the sheet looks like afterwards. LS pins ARGUMENT POSITION: `swapTeamsInActiveMatch` is `(matchId, sessionId, …)` while `swapPlayerInActiveMatch` is `(matchId, outPlayerId, inPlayerId, sessionId, …)`. Every parameter is a string, so a transposition type-checks, returns success, and swaps the wrong player — or names the outgoing player as the incoming one in the broadcast every affected player receives. "The action was called" cannot see any of it |
| `use-organizer-alerts.test.ts` | **Suite OAL** — the prefix `OA` is claimed by `oauth-actions.test.ts`, and two suites sharing one makes a red ID ambiguous in CI output. This hook is the only place an organizer learns mid-session that a player has checked out or is disputing a score, so a dropped card is not cosmetic: it is a published match with a player who is not in the building. It fetches on four independent triggers — mount, a 45 s poll, visibility recovery, and a caller-driven `refreshInbox()` — three of which fire within a few hundred milliseconds of each other on a phone waking from lock, so out-of-order responses are routine rather than rare and `fetchSeq` is the only thing standing between them and a resurrected card |
| `use-organizer-courts.test.ts` | **Suite OC — the realtime channel that must open ONCE.** OC-7/OC-8/OC-9 pin the three separable ways to break CLAUDE.md's subscription-stability rule: a render-scoped value in the effect's dependency array, a fetch callback that stops being referentially stable, and the `fetchCourtsRef.current` indirection replaced by the callback itself. The production symptom is not an exception — it is a court status change that lands during the ~1 s the socket is re-joining and never arrives, so the organizer reads "in use" for a court that finished two matches ago, nothing logs, and the next manual refresh hides it |
| `use-organizer-queue.test.ts` | **Suite OQ — two channels (`queue_entries` and `profiles`) opened from one effect,** with the same stability requirement and the same silent symptom: a player who joins during the re-join window simply never appears in the panel until some later event happens to refetch. OQ-12/OQ-13/OQ-14 pin the three separable breakages. OQ also covers the 07/25 blanking class from the organizer side — an auth-degraded read returning success-with-zero-rows must NOT clear a populated panel, while a genuinely empty result must |
| `use-session-data.test.ts` | **Suite SD — the player-side data spine** behind every `/play/[id]` screen: three independent reads (courts, active matches, waitlist), each with its own fetch, ref-callback and monotonic `fetchSeq`, all fed by one consolidated subscription set. Delete a sequence re-check and an overlapping pair resolving out of order paints the OLDER queue over the newer one, with nothing erroring and the next realtime event papering over it. SD asserts BOTH halves of the blanking fork — a genuinely empty result must clear, an error and an anon-empty result must not — because they arrive through the same code path. SD-23 pins that `TOKEN_REFRESHED` triggers a refetch, which is also what drives `use-auth-recovery-refetch.ts` |
| `use-session-completed-players.test.ts` | **Suite CP — the history-swap candidate list** behind step 2 of `FixRecordSheet`, where an organiser correcting a finished match picks the player who actually played. The list IS the exclusion: `.neq("id", excludeMatchId)` is the whole rule, and binding it to the wrong column or the wrong value offers the organiser the very players they are trying to swap out — or drops the session scope and lists strangers. The three per-row stats are computed in memory from a four-query pipeline rather than read from a view, so a win credited to the wrong team is silent, and those numbers are how a human tells two same-named players apart before committing a permanent correction |

**Where the layer stands, and what these suites do not prove.** `src/app/actions/`, `src/lib/` and
`src/hooks/` each now have a suite naming every module. Recompute what is left with:
...
It prints `src/lib/fonts.ts` — two exported string constants and no behaviour, so there is no
mutation of it a test could catch that `npx tsc --noEmit` does not already catch — and
`src/hooks/use-auth-recovery-refetch.ts`, which is reached only through `use-session-data`, is
driven by SD-23, and is ratcheted in `coverage.include` regardless. Every entry added to that list
was written against one question — *if the behaviour this test names were broken, would this test go
red?* — and answered by applying a mutation to the source, watching the named IDs fail, and
...
| `close-session.test.ts`    | B     | `closeSession`: co-org access, idempotency, concurrent-close claim (Test 3b), match cancellation, queue drain, wrapped stats     |
...
| `court-management.test.ts` | CX    | **The three court actions against real rows** — the session-scope binding proven against the database rather than a recorded mock: an organizer of session A cannot update (CX-5) or delete (CX-7) a court in session B. Both carry a POSITIVE CONTROL in the same test — the same caller, same call shape, own court, must really succeed — so an action that is simply broken for everyone cannot satisfy them. CX-8 pins the actual contract that a zero-row write still reports success |
  ...
  app/
    actions/
      ...
      sessions.ts      # createSession, joinAsCoOrganizer, toggleAutoMatchmaking, updateSessionSettings,
                       #   closeSession (ended_at claim before Wrapped; loser: closeInFlight | alreadyClosed; is_active CAS on the flip)
