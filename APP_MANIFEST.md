...
**Suites (143 passed + 1 skipped across 15 files — TDD back-fill 2026-05-12):**
...
| **`session-lifecycle.test.ts` (Suite K, NEW)** | 13              | `createSession` (passcode unique, length, trigger, unauth), `joinAsCoOrganizer` (idempotent, primary-org rejection, generic invalid msg, closed-session rejection), `toggleAutoMatchmaking` (atomic flip, organiser auth)                                                                                 |
| **`queue-join.test.ts` (Suite L, NEW)**        | 8               | `joinQueueAction` happy + Inherited Games floor, returning 'left' rejoin, drafted/on_deck/playing rejection, unauth, `checkoutPlayer` draft cleanup                                                                                                                                                       |
| **`manual-and-swap.test.ts` (Suite M, NEW)**   | 12 (+1 skipped) | `createManualMatchAction` happy + 3 negative paths, `swap_player_in_match` sticky-rule (manual stays, modified stays), `swapPlayerInMatch` action guards (PLAYER_UNAVAILABLE / PLAYER_NOT_IN_MATCH / MATCH_STARTED), `swap_match_players` origin flips. **M-5 SKIPPED — see Section 11 regression note.** |
