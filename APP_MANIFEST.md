...
§3.1, where it is not a characterisation but arithmetic, and disagreed with every other site stating the
figure (`git grep -n "2 ever reached a court\|10 cleared by hand\|10 of 12"` — do not maintain the tally
in prose, this sentence said "four" and the grep returns six). A squash message cannot be amended, so
`61e942b`'s body is permanent and wrong; these documents are the correctable record.
...
flip, is **empty** (migration stamped `20260815133945`, hours after the session ended). Worse, defect 4
