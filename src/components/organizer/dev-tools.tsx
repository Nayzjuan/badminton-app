...
export function DevTools({ sessionId }: DevToolsProps) {
  ...
  async function handleClear() {
    if (!confirmNuke) {
      setConfirmNuke(true);
      if (nukeTimerRef.current) clearTimeout(nukeTimerRef.current);
      nukeTimerRef.current = setTimeout(() => setConfirmNuke(false), 3000);
      return;
    }
  ...
  return (
    <div className="relative">
      ...
      {expanded && (
        <div
          className="absolute right-0 top-full mt-2 w-80 rounded-xl border-2 border-dashed border-red-300
            ...
            <div className="space-y-1.5">
              ...
              >
                {seeding ? (
                  <>
                    <span
                      className="inline-block h-3.5 w-3.5 border-2 border-amber-600
              ...
              <p className="text-[10px] text-muted-foreground px-1">
                Creates dummy players with randomized skill levels, wait times (3-5 bottlenecks &gt;
                20min), and game counts.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                {seedingNamed ? (
                  <>
                    <span
                      className="inline-block h-3.5 w-3.5 border-2 border-emerald-600
                                     border-t-transparent rounded-full animate-spin"
                    />
              ...
              <p className="text-[10px] text-muted-foreground px-1">
                Creates 16 specific players (Carl, Stelle, Miggy, JV, etc.) as new joiners with 0
                games played.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                {clearing ? (
                  <>
                    <span
                      className="inline-block h-3.5 w-3.5 border-2 border-red-600
                                     border-t-transparent rounded-full animate-spin"
                    />
                    Clearing...
                  </>
