# MEMORY — ARCHIVE (session history, 2026-05 → 2026-08)

> Closed narrative history split out of `MEMORY.md` on 2026-08-19, when that file had reached
> 678 KB and no longer fit in a context window alongside the code it describes.
>
> **Nothing here is current state.** Every item in this file is shipped, closed, declined, or
> superseded. Current state — what is in flight, what is open, and the migration/prod-stamp
> table — lives in `MEMORY.md`. The open items rescued out of the "STANDING TO-DO" section
> below were carried forward there; the rest is kept only as the record of how each decision
> was reached.
>
> Read this file by search (`grep -n`), never end-to-end.
...
The four sections below were current state in `MEMORY.md` until PR #74 squash-merged to `main` as
`5949f5e` and Vercel production reached READY (`dpl_7K7XtwFPEbMXPUCvyMTmYaesAnpL`, target
`production`, aliases attached, `aliasError: null`). All three migrations the branch carried were
already stamped on prod **before** the merge, so the merge shipped TypeScript only. Archived
verbatim as the record of how each was reached; the parts that still bind future work were carried
to `MEMORY.md` → *STANDING CONSTRAINTS*.
...
## Part 2 — the 2026-08-13 STANDING TO-DO list, as it stood when it was retired
...
## Part 3 — sessions 2026-07-24 back to 2026-05-19
